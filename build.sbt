...
lazy val fragnosticConfPropsSettings = Seq(
...
lazy val fragnosticConfPropsProject = Project(
  id = "fragnostic-conf-props-project",
  base = file(".")).settings(
    fragnosticConfPropsSettings ++ Seq(
...
lazy val fragnosticConfProps = Project(
  id = "fragnostic-conf-props",
  base = file("fragnostic-conf-props")).settings(fragnosticConfPropsSettings ++ Seq(
...
lazy val mavenCentralFrouFrou = Seq(
  homepage := Some(url("http://www.fragnostic-conf-props.org/")),
  startYear := Some(2022),
  licenses := Seq(("BSD", url("http://github.com/fragnostic-conf-props/fragnostic-conf-props/raw/HEAD/LICENSE"))),
  pomExtra := pomExtra.value ++ Group(
    <scm>
      <url>http://github.com/fragnostic-conf-props/fragnostic-conf-props</url>
      <connection>scm:git:git://github.com/fragnostic-conf-props/fragnostic-conf-props.git</connection>
    </scm>
    <developers>
      <developer>
        <id>fbrule</id>
